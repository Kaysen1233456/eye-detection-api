...
def get_seq_frames(video_size, num_frames, clip_idx, num_clips, start_index=0, max_frame=-1):
    if max_frame == -1:
        ...
        video_size = max_frame - start_index + 1
...
def decode(
    ...
    if backend in ["pyav", "torchvision"]:
        ...
        if sparse:
            if total_frames:
            ...
            elif INPUT_CLIP: # 输入clip
                # >- 输入clip
                if LAST_FRAMES:
                    # >- 取最后连续帧
                    actual_start = max(0, len(frames)-num_frames)
                    seq = np.arange(actual_start, len(frames))
                    seq = np.pad(seq, (num_frames - len(seq), 0), 'edge')
                ...
                # >- 输入整个视频
                actual_total_frame = len(frames)
                if video_frame_count != actual_total_frame:
                    # 计算原始起止帧在总帧数中的比例
                    start_ratio = start_frame / video_frame_count
                    end_ratio = end_frame / video_frame_count
                    
                    # 按比例重新计算实际视频中的起止帧位置 (四舍五入取整)
                    start_frame = round(start_ratio * actual_total_frame)
                    end_frame = round(end_ratio * actual_total_frame)
                    video_frame_count = actual_total_frame

                # >- 最终边界检查
                start_frame = max(0, min(start_frame, video_frame_count - 1))
                end_frame = max(start_frame, min(end_frame, video_frame_count - 1))  
                
                if VISUAL or EVALUATE:
                    # >- 取指定连续帧
                    seq = np.arange(start_frame, end_frame+1)
                    seq  = np.pad(seq, (0, num_frames - len(seq)), 'edge')
                    frames = frames.get_batch(seq)
                elif LAST_FRAMES:
                    # >- 取最后连续帧
                    actual_start = max(start_frame, end_frame - num_frames + 1)
                    seq = np.arange(actual_start, end_frame + 1)
                    seq = np.pad(seq, (num_frames - len(seq), 0), 'edge')
                    frames = frames.get_batch(seq)
                else:
                    seq = get_seq_frames(len(frames), num_frames, clip_idx, num_clips, start_index=start_frame, max_frame=end_frame)
                    frames = frames.get_batch(seq)
                ####################################################################################################################
        else:
