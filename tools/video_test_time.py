...
class FatigueDataset(Dataset):
    def __init__(self, annotation_path, cfg):
        ...
        for anno_idx, anno in enumerate(tqdm(self.annotations[:1])):
...
@torch.no_grad()
def perform_test_fatigue(model, loader, cfg):
    """
    For classification:
    Perform mutli-view testing that uniformly samples N clips from a video along
    its temporal axis. For each clip, it takes 3 crops to cover the spatial
    ...
    print(f'video_time:{video_time}s')
    # print(1)
    ...
    # # 1. 计算性能指标
    # metrics = evaluate_fatigue_detection(predictions, fatigue_labels)
...
def video_fatigue_test(cfg):
    ...
    perform_test_fatigue(model, loader, cfg)
...
def evaluate_fatigue_detection(predictions, fatigue_labels):
    """
